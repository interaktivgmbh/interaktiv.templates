...
ifdef CI
	...
	@uv venv --clear --python=3.10 $(VENV_FOLDER)
endif
...
i18n: $(VENV_FOLDER) ## Update locales
	...
	@$(BIN_FOLDER)/python -m interaktiv.templates.locales
...
test-coverage: $(VENV_FOLDER) ## run tests with coverage
	@$(BIN_FOLDER)/pytest --cov=interaktiv.templates --cov-report term-missing
