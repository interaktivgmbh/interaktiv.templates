...
class TestThumbnailServicePost(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.service = TemplateThumbnailPost()
    ...
    def test_thumbnail_post__create_and_assign_template_thumbnail(self, mock_get_thumbnail):
        ...
        provideUtility(mock_storage, IStorage)

        # do it
        self.service._create_and_assign_template_thumbnail(self.template)

        # post condition
        self.assertEqual(len(self.template.objectValues()), 1)

        # cleanup
        api.content.delete(self.template.objectValues()[0])
        self.template.template_thumbnail = None
        self.template.reindexObject(idxs=["template_thumbnail"])

    @patch.object(TemplateThumbnailPost, '_get_thumbnail_image', return_value=b"fake-image-data")
    def test_thumbnail_post__replace_thumbnail(self, mock_get_thumbnail_image):
        # setup
