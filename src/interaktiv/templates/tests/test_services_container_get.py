...
from interaktiv.templates.services.templatescontainer.get import InteraktivTemplatesContainerGet
from interaktiv.templates.testing import INTERAKTIV_TEMPLATES_FUNCTIONAL_TESTING


class TestTemplatesContainerGet(TestCase):
    layer = INTERAKTIV_TEMPLATES_FUNCTIONAL_TESTING
    service: InteraktivTemplatesContainerGet = None

    def setUp(self):
        super().setUp()

        self.service = InteraktivTemplatesContainerGet()
    ...
    def test_template_container_get__containers_result(self):
        # setup
        self.service.request.form = {
            'url': urlparse(self.template.absolute_url()).path}
        ...
        self.assertEqual(
            result.get('containers')[0].get('id'),
            self.template_container.getId())
        self.assertEqual(
            result.get('containers')[0].get('path'),
            '/plone/templates_container')

    def test_template_container_get__no_template_containers(self):
        # setup
        with patch('plone.api.content.find', return_value=[]):
            self.service.request.form = {
                'url': urlparse(self.template.absolute_url()).path}
    ...
    def test_template_container_get__nearest_container(self):
        # setup
        self.service.request.form = {
            'url': urlparse(self.template.absolute_url()).path}
        ...
        # post condition
        self.assertEqual(
            result.get('nearest_container'),
            self.template_container.absolute_url())
    ...
    def test_get_nearest_template_container__no_container(self):
        ...
        # do it
        result = self.service._get_nearest_template_container(
            self.template, template_containers)
    ...
    def test_get_nearest_template_container__nearest_container_returned(self):
        ...
        content.getPhysicalPath = Mock(
            return_value=('', 'site', 'path', 'content'))

        container1 = Mock(spec=ICatalogBrain)
        container1.getPhysicalPath = Mock(
            return_value=('', 'site', 'path', 'a'))
        container1.getObject = Mock(return_value=container1)

        container2 = Mock(spec=ICatalogBrain)
        container2.getPhysicalPath = Mock(
            return_value=('', 'site', 'path', 'content', 'b'))
        container2.getObject = Mock(return_value=container2)

        container3 = Mock(spec=ICatalogBrain)
        container3.getPhysicalPath = Mock(
            return_value=('', 'site', 'path', 'c'))
    ...
    def test_serialize_container(self):
        # do it
        result = self.service._serialize(
            brain=uuidToCatalogBrain(self.template_container.UID()))
